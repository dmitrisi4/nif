...
})({"6GLrr":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "771b50dc43a7421f";
"use strict";
...
},{}],"64jdM":[function(require,module,exports) {
document.addEventListener("DOMContentLoaded", function() {
    ...
    //	init
    if (contactInputName.value.length === 0 || contactInputEmail.value.length === 0 || contactInputComment.value.length === 0) contactBtn.disabled = true;
    if (formFirstName.value.length === 0 || formLastName.value.length === 0 || formEmail.value.length === 0 || formTel.value.length === 0) regFormBtn.disabled = true;
    const radioBtnGroup = orderChangeForm.plan;
    let prev = null;
    for(let i = 0; i < radioBtnGroup.length; i++)radioBtnGroup[i].addEventListener("change", function() {
        prev && (regFormBtn.disabled = false);
        if (this !== prev) {
            prev = this;
    ...
    registrationEmail.addEventListener("input", ()=>{
        ...
        if (test) setValidEmailStateFormRegUser();
        else setInvalidEmailStateFormRegUser();
    });
    registrationPassword.addEventListener("input", ()=>{
        if (registrationPassword.value.length < 6) setInvalidPasswordStateFormRegUser();
        else setValidPasswordStateFormRegUser();
    });
    formFirstName.addEventListener("input", ()=>{
        const test = nameMatch(formFirstName.value);
        if (test) setValidFirstNameStateFormReg();
        else setInvalidFirstNameStateFormReg();
    });
    formLastName.addEventListener("input", ()=>{
        const test = nameMatch(formLastName.value);
        if (test) setValidLastNameStateFormReg();
        else setInvalidLastNameStateFormReg();
    });
    formEmail.addEventListener("input", ()=>{
        const test = formEmail.value.length === 0 || emailRegExp.test(formEmail.value);
        if (test) setValidEmailStateFormReg();
        else setInvalidEmailStateFormReg();
    });
    ...
    contactInputName.addEventListener("input", ()=>{
        ...
        if (test) setValidContactNameStateFormReg();
        else setInvalidContactNameStateFormReg();
    });
    contactInputEmail.addEventListener("input", ()=>{
        ...
        if (test) setValidContactEmailStateFormReg();
        else setInvalidContactEmailStateFormReg();
    });
    contactInputComment.addEventListener("input", ()=>{
        const test = contactInputComment.value.length <= 200;
        if (test) setValidContactCommentStateFormReg();
        else setInvalidContactCommentStateFormReg();
    });
});

},{}]},["6GLrr","64jdM"], "64jdM", "parcelRequire7bbc")

//# sourceMappingURL=index.43a7421f.js.map
